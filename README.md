...
- **O fluxo**: `Para avaliar` → `Avaliado` → `A pagar` → `Para guardar` → `Para inserir` → `Inserido`, com `Recusada` como saída terminal a partir de `Avaliado`. Avaliar exige link e valor. Confirmar o pagamento **não** exige comprovante: no crédito não existe transferência a comprovar, e no pix a prova que vale está no extrato do banco, não numa imagem que qualquer um anexa — o anexo continua aceito (mesmo caminho de upload das capas de evento) e, sem ele, a tela pede uma confirmação. Os dois últimos passos são de prateleira
...
- **Os valores são congelados na linha**: o bruto, os percentuais usados e os dois resultados já calculados. Mudar a política no ano que vem não reescreve o que foi oferecido hoje. Contas em centavos inteiros, com arredondamento meio-para-cima — R$ 33,33 a 50% dá R$ 16,67, e a diferença fica com o cliente
- **O percentual é negociável por OS.** O formulário de avaliar já vem com o padrão da loja (60% crédito, 50% pix, o `DEFAULT` das colunas — não uma constante repetida no código), e admin ou avaliador troca qualquer um dos dois, de 1 a 100, antes de concluir. Os dois valores aparecem **enquanto se digita**, para ninguém confirmar uma proposta sem ver quanto ela paga; a prévia é um espelho da conta do servidor (`frontend/src/app/lib/proposta.ts`), testado contra os mesmos casos do backend, e o número que vale é sempre o que o servidor grava. O histórico registra os percentuais junto do valor — meses depois, "por que esta OS pagou 55%?" tem resposta. **Mudar o percentual de uma proposta já enviada não se faz no lugar**: volta-se um passo para `Para avaliar` (com motivo), o que invalida o link do cliente de propósito
...
- **Excluir é o único ato que não deixa a OS para trás** — e por isso é o mais travado. Só **admin**, e a fronteira é o dinheiro: `Para avaliar`, `Avaliado` e `Recusada` podem ser excluídos; de `A pagar` em diante, não, porque a loja se comprometeu, pagou ou já guardou as cartas. Para desfazer um negócio o caminho continua sendo `voltar`, um passo por vez com motivo. Fora de `Para avaliar` o motivo é obrigatório, e sempre é preciso digitar o **código da OS** à mão (comparado como a busca compara: sem hífen, sem caixa), no mesmo pedágio que anonimizar uma conta cobra com o nome da pessoa. O comprovante sai do S3 junto, senão viraria anexo órfão. **O link do cliente para de abrir na hora**
- **O que sobra de uma OS excluída** é uma linha em `avaliacao_exclusoes` (migration 021): código, contato, em que pé estava, valores e quem excluiu, com motivo. Não copia e-mail, link da planilha nem chave pix — guardar dado pessoal de um negócio que a loja decidiu apagar seria o contrário do que apagar significa. A tabela não tem chave estrangeira para `avaliacoes` de propósito: a linha dela nasce quando a de lá morre
...
POST   /api/avaliacoes/:id/avaliar         # link + valor + percentuais; cria o link público
...
DELETE /api/avaliacoes/:id                 # só admin, antes do pagamento; exige o código
